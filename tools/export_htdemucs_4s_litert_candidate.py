#!/usr/bin/env python3
"""Export the official four-stem fixed-window HTDemucs neural core to LiteRT.
...
MODEL_FAMILY_SPECS = {
    "htdemucs-6s": {
        "family": "HTDemucs-6s",
        "sourceOrder": ("drums", "bass", "other", "vocals", "guitar", "piano"),
        "modelIds": {
            "smoke_2s": "htdemucs_6s_core_smoke_2s_fp32_v1_0_0",
            "canonical_7p8s": "htdemucs_6s_core_canonical_7p8s_fp32_v1_0_0",
        },
        "weightBytes": 54_885_744,
        "weightSha256": "d2a1745f0744721f6b8ca5bf469b67c651ea5ed1b52998cab033b2158609d411",
        "metadataBytes": 10_398,
        "metadataSha256": "72d7b4739ba40c8ff1d697404232edd335f397cedbf1bb88eec0034bdbab153e",
        "bagManifestFileName": "htdemucs_6s.yaml",
        "bagManifestBytes": 21,
        "bagManifestSha256": "207405151270af8fd81c2373c25d27950916682ac91dca7884a11ce13dad6f58",
        "stateTensorCount": 525,
        "stateValueCount": 27_414_996,
    },
    "htdemucs-4s": {
        "family": "HTDemucs-4s",
        "sourceOrder": ("drums", "bass", "other", "vocals"),
        "modelIds": {
            "smoke_2s": "htdemucs_4s_core_smoke_2s_fp32_v1_0_0",
            "canonical_7p8s": "htdemucs_4s_core_canonical_7p8s_fp32_v1_0_0",
        },
        "weightBytes": 84_025_440,
        "weightSha256": "d9fa14133cfcc034a6758923bb3a8ca9f8dfd0b582134643bbf83f72c17576dd",
        "metadataBytes": 12_087,
        "metadataSha256": "12540373de858920b60002ebfbe17738860dbf2e89df625dc3b7875af2d28491",
        "bagManifestFileName": "htdemucs.yaml",
        "bagManifestBytes": 21,
        "bagManifestSha256": "239c445d0b14454d541ad8bd9bb271c9e536d267e8a4625208744cbb2e7bb66c",
        "stateTensorCount": 533,
        "stateValueCount": 41_984_456,
    },
}
OFFICIAL_MODEL_IDS = frozenset(
    model_id
    for family in MODEL_FAMILY_SPECS.values()
    for model_id in family["modelIds"].values()
)

# These globals retain the public helper API used by the existing validator.
# main() selects one immutable family before loading or converting a model.
SOURCE_ORDER = MODEL_FAMILY_SPECS["htdemucs-6s"]["sourceOrder"]
EXPECTED_WEIGHT_BYTES = MODEL_FAMILY_SPECS["htdemucs-6s"]["weightBytes"]
EXPECTED_WEIGHT_SHA256 = MODEL_FAMILY_SPECS["htdemucs-6s"]["weightSha256"]
EXPECTED_METADATA_BYTES = MODEL_FAMILY_SPECS["htdemucs-6s"]["metadataBytes"]
EXPECTED_METADATA_SHA256 = MODEL_FAMILY_SPECS["htdemucs-6s"]["metadataSha256"]
EXPECTED_BAG_MANIFEST_FILE_NAME = MODEL_FAMILY_SPECS["htdemucs-6s"]["bagManifestFileName"]
EXPECTED_BAG_MANIFEST_BYTES = MODEL_FAMILY_SPECS["htdemucs-6s"]["bagManifestBytes"]
EXPECTED_BAG_MANIFEST_SHA256 = MODEL_FAMILY_SPECS["htdemucs-6s"]["bagManifestSha256"]
...
def parse_args() -> argparse.Namespace:
    ...
    official_model_id = MODEL_FAMILY_SPECS[args.model_family]["modelIds"][args.profile]
    if args.derived_weight_manifest is None:
        ...
        if args.model_family != "htdemucs-6s":
            parser.error("Derived weights are currently supported only for htdemucs-6s")
...
def configure_model_family(name: str) -> dict[str, Any]:
    global SOURCE_ORDER
    global EXPECTED_WEIGHT_BYTES
    global EXPECTED_WEIGHT_SHA256
    global EXPECTED_METADATA_BYTES
    global EXPECTED_METADATA_SHA256
    global EXPECTED_BAG_MANIFEST_FILE_NAME
    global EXPECTED_BAG_MANIFEST_BYTES
    global EXPECTED_BAG_MANIFEST_SHA256

    spec = MODEL_FAMILY_SPECS[name]
    SOURCE_ORDER = spec["sourceOrder"]
    EXPECTED_WEIGHT_BYTES = spec["weightBytes"]
    EXPECTED_WEIGHT_SHA256 = spec["weightSha256"]
    EXPECTED_METADATA_BYTES = spec["metadataBytes"]
    EXPECTED_METADATA_SHA256 = spec["metadataSha256"]
    EXPECTED_BAG_MANIFEST_FILE_NAME = spec["bagManifestFileName"]
    EXPECTED_BAG_MANIFEST_BYTES = spec["bagManifestBytes"]
    EXPECTED_BAG_MANIFEST_SHA256 = spec["bagManifestSha256"]
    return spec
...
def main() -> int:
    args = parse_args()
    model_family_spec = configure_model_family(args.model_family)
    ...
    report: dict[str, Any] = {
        ...
        "modelFamily": model_family_spec["family"],
    ...
    try:
        ...
        bag_manifest = canonical_weight.parent / EXPECTED_BAG_MANIFEST_FILE_NAME
        ...
        expected_shapes = {
            ...
            "frequencyOutput": [
                ...
                4,
                2048,
                math.ceil(args.samples / 1024),
            ],
            "waveformOutput": [1, len(SOURCE_ORDER), 2, args.samples],
            "combinedOutput": [1, len(SOURCE_ORDER), 2, args.samples],
        }
        actual_shapes = {
