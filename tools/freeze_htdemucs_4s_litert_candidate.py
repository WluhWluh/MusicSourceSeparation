#!/usr/bin/env python3
"""Freeze and verify the official four-stem canonical HTDemucs LiteRT candidate."""
...
MODEL_SPECS = {
    ...
    "htdemucs-4s": {
        ...
        "sourcePaths": {
            "canonicalWeight": "models/demucs/official-hf/htdemucs/955717e8.safetensors",
            "metadata": "models/demucs/official-hf/htdemucs/955717e8.json",
            "bagManifest": "models/demucs/official-hf/htdemucs/htdemucs.yaml",
        },
    },
}
EXPECTED_INPUTS = [
...
SHARED_SOURCE_PATHS = {
    ...
    "exportScript": "tools/export_htdemucs_4s_litert_candidate.py",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-family", choices=("htdemucs-4s",), default="htdemucs-4s")
...
def expected_outputs(source_count: int) -> list[tuple[str, list[int], list[str]]]:
    return [
        (
            "serving_default_output_0_output",
            [1, source_count, 4, 2048, SPECTRUM_FRAME_COUNT],
            ["batch", "stem", "feature", "frequency", "frame"],
        ),
        (
            "serving_default_output_1_output",
            [1, source_count, 2, SAMPLE_COUNT],
...
def build_manifest(
    ...
    outputs_contract = expected_outputs(len(source_order))
    ...
    require(len(graph["outputs"]) == len(outputs_contract), "Output count mismatch")
    inputs = [bind_tensor(value, expected) for value, expected in zip(graph["inputs"], EXPECTED_INPUTS, strict=True)]
    outputs = [bind_tensor(value, expected) for value, expected in zip(graph["outputs"], outputs_contract, strict=True)]
    ...
    source_paths = {**model_spec["sourcePaths"], **SHARED_SOURCE_PATHS}
...
def main() -> int:
    ...
    model_spec = MODEL_SPECS[args.model_family]
    requested_candidate_root = args.candidate_root or (
        Path("models/demucs/generated") / model_spec["modelId"]
    )
    candidate_root = (
        requested_candidate_root
        if requested_candidate_root.is_absolute()
        else repo_root / requested_candidate_root
    ).resolve()
    output = args.output or candidate_root / "candidate-manifest.json"
    if not output.is_absolute():
        output = repo_root / output
    output = output.resolve()
    manifest = build_manifest(repo_root, candidate_root, model_spec)
