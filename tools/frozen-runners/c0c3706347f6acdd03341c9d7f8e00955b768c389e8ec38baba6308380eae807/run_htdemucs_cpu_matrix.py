...
def device_identity(args: argparse.Namespace) -> dict[str, Any]:
    ...
    identity["hardwareSerial"] = common.adb(
        args,
        "shell",
        "getprop",
        "ro.serialno",
...
def remote_run_path(model: ModelIdentity, duration_seconds: int, run_id: str) -> str:
...
def run_variant(
    ...
    run_id = f"{args.device_label}-{model.variant}-{slug}-{duration_seconds}s"
    remote_run = remote_run_path(model, duration_seconds, run_id)
    ...
    log_stem = f"instrumentation-{args.device_label}-{duration_seconds}s"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-hardware-serial")
    ...
    parser.add_argument(
        "--source-root",
        type=Path,
        default=Path(r"C:\Users\User\Documents\MusicPlayer\test\mp3_no_gapless_metadata"),
    )
    parser.add_argument(
        "--canonical-root",
        type=Path,
...
def main() -> int:
    ...
    if (
        args.expected_hardware_serial is not None
        and identity["hardwareSerial"] != args.expected_hardware_serial
    ):
        raise ValueError(
            f"Selected hardware serial is {identity['hardwareSerial']!r}, "
            f"expected {args.expected_hardware_serial!r}"
        )
    local_apks = {
    ...
    try:
        ...
        progress_path = args.output_root / f"matrix-{args.duration_seconds}s.json"
