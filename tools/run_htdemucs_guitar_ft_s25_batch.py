#!/usr/bin/env python3
"""Run three bounded 30-second guitar-ft CPU diagnostics on the S25."""
...
MODEL_FILE = "htdemucs_6s_guitar_ft.core.canonical_7p8s.fp32.tflite"
MODEL_BYTES = 117_729_544
MODEL_SHA256 = "ab632a5a024033d557eabb716f8829230532e8e5b4cd7ba146812a301f89b9a5"
SAMPLE_RATE = 44_100
DURATION_SECONDS = 30
...
def stage_model(args: argparse.Namespace, artifact: Path) -> dict[str, Any]:
    if artifact.stat().st_size != MODEL_BYTES or sha256_file(artifact) != MODEL_SHA256:
        raise ValueError("Local guitar-ft LiteRT artifact identity mismatch")
    ...
    if not remote_hash.startswith(MODEL_SHA256):
        raise ValueError(f"Remote guitar-ft model SHA mismatch: {remote_hash}")
    size = adb(args, "shell", "stat", "-c", "%s", remote).stdout.strip()
    if size != str(MODEL_BYTES):
        raise ValueError(f"Remote guitar-ft model size mismatch: {size}")
    return {
        "modelId": MODEL_ID,
...
def validate_run(
    ...
    if (
        model.get("variant") != "guitar-ft"
        or model.get("modelId") != MODEL_ID
        or model.get("sha256") != MODEL_SHA256
        or model.get("diagnosticOnly") is not True
        or model.get("researchOnly") is not True
        or model.get("hostAdmissionStatus") != "not-admitted"
    ):
        raise ValueError("S25 report does not bind the diagnostic guitar-ft identity")
    source = report["source"]
    if source["fileSha256"] != source_sha:
        raise ValueError("S25 source MP3 SHA mismatch")
    if source["selectedFrames"] != SELECTED_FRAMES:
        raise ValueError("S25 run did not select exactly 30 seconds")
    ...
    if attempt.get("status") != "complete" or attempt.get("expectedWindows") != 6:
        raise ValueError("S25 attempt did not complete the expected six windows")
...
def process_track(
    ...
    target = output_root / "tracks" / slug / "s25-cpu-30s"
    if target.is_dir():
        validation = validate_run(target, source_sha, expected_selected_pcm_sha)
        return {
            "category": category,
            "slug": slug,
    ...
    remote_audio_name = f"guitar-ft-30s-{slug}{source.suffix.lower()}"
    remote_audio = f"{REMOTE_ROOT}/audio/{remote_audio_name}"
    run_id = f"guitar-ft-{slug}-30s"
    remote_run = (
        f"{REMOTE_ROOT}/htdemucs-canonical-e2e/cpu/guitar-ft/30s/{run_id}"
    )
    ...
    validation = validate_run(target, source_sha, expected_selected_pcm_sha)
    write_json(log_root / "device-envelope.json", {
        "category": category,
        "sourceFileName": source.name,
...
def main() -> int:
    ...
    write_json(output_root / "device.json", device)
    ...
    progress_path = output_root / "batch-progress.json"
    write_json(progress_path, progress)
    try:
        for category, file_name in TRACKS:
    ...
    progress["finalSnapshot"] = device_snapshot(args)
    progress["aggregateAudioSeconds"] = DURATION_SECONDS * len(TRACKS)
